...
- прямые upload URL в консоль не печатаются, чтобы не засорять вывод.
...
- Подменяет `fetch`-транспорт сценарием-обвязкой и отслеживает upload-запросы без шумного дампа URL.
