...
Сценарий проверяет метод `api.getVideoAttachmentDetails(videoToken)`.

Команды:

- `/videoDetails` — загрузить demo-видео из `MAX_UPLOAD_PROGRESS_VIDEO_PATH`, взять полученный token и запросить details.
- `/videoDetails <token>` — запросить details по готовому token без новой загрузки файла.

В чат отправляется краткое summary: token, размер, длительность, наличие thumbnail и список доступных URL-форматов. Полный ответ API печатается в терминале через `console.dir`.

При загрузке demo-видео используется тот же progressbar и отмена по `Esc`, что и в upload-сценарии.

## Что проверяет сценарий `40-reply-keyboard-data`

Сценарий проверяет отправку `reply_keyboard` и входящие attachments после нажатия reply-кнопок.
