...
const pluralRules: Readonly<
  Record<AvailableRuleLanguages, (n: number) => number>
> = {
...
const AVAILABLE_RULE_LANGUAGES = Object.keys(
  pluralRules,
) as readonly AvailableRuleLanguages[];

const mapping: Readonly<
  Record<AvailableRuleLanguages, readonly LanguageCode[]>
> = {
  english: [
    'da',
    'de',
...
function findRuleLanguage(languageCode: string): AvailableRuleLanguages {
  const result = AVAILABLE_RULE_LANGUAGES.find((key) =>
    mapping[key].includes(languageCode),
  );
  if (!result) {
