...
> {
  private context: IStepContextOptions<C, P>['context'];
  private steps: IStepContextOptions<C, P>['steps'];

  private stepChanged = false;

  public constructor(options: IStepContextOptions<C, P>) {
   ...
   */
  public get current(): StepSceneHandler<C, P> | undefined {
  ...
  public async reenter(): Promise<void> {
    ...
    if (
      this.context.scene.lastAction !== LastAction.LEAVE &&
      !this.stepChanged
    ) {
   ...
   * ```ts
   * ctx.scene.step.go(3);
   * ctx.scene.step.go(3, { silent: true });
   * ```
   */
  public go(
    stepId: number,
    { silent = false }: IStepContextGoOptions = {},
  ): Promise<void> {
    this.stepId = stepId;
   ...
   * ```ts
   * ctx.scene.step.next();
   * ctx.scene.step.next({ silent: true });
   * ```
   */
   ...
   * ctx.scene.step.previous({ silent: true });
   * ```
   */
