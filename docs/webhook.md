...
Встроенный runtime поднимает локальный HTTP-сервер. В production разместите перед ним HTTPS reverse proxy или load balancer, который завершает TLS и передаёт запросы только на этот сервер; не публикуйте HTTP-порт напрямую.
...
- JSON body и обязательные поля update: непустой `update_type` и числовой `timestamp`;
